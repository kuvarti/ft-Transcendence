...
export class Auth42Controller {
  ...
  async login(@Res() response: Response, @Req() request: Request) {
    ...
    if (!userToLogin.success) {
      return response.redirect(
        this.configService.get<string>('UI_BASE_URL') + '/redirection-auth42/' + "none" + '/' + (await userToLogin).success + '/' + (await userToLogin).message,
      );
    }

    const result = await this.authService.createAccessToken(userToLogin.data);
    return response.redirect(
      this.configService.get<string>('UI_BASE_URL') + '/redirection-auth42/' + (await result).data.token + '/' + (await result).success + '/' + (await result).message,
    );
  }
  ...
  async register(@Res() response: Response, @Req() request: Request) {
    ...
    if (!(await registerResult).success){
      return response.redirect(
        this.configService.get<string>('UI_BASE_URL') +'/redirection-auth42/' + "none" + '/' + (await registerResult).success + '/' + (await registerResult).message,
      );
    }
